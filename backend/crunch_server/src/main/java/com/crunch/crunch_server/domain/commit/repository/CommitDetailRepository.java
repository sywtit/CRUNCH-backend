...
public interface CommitDetailRepository extends JpaRepository<PostLineDetail,Integer> {
	...
	@Query(value = "SELECT * FROM postLineDetail AS p1, (SELECT lineNum, max(id) AS id FROM postLineDetail GROUP BY lineNum HAVING lineNum < :afterPostLength ORDER BY id desc)"
